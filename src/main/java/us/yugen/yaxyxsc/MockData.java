        ...
        return IntStream.range(0, amount).mapToObj((i) -> {
            ...
            if (new Random().nextInt(100) >= -1) {
                for (var tag : Tag.values()) {
                    ...
                    final List<ShoppingList> shoppingLists = DataStore.SHOPPING_LISTS_BY_TAG.get(randomShoppingList.tag);
                    if (null == shoppingLists) {
                        final ArrayList<ShoppingList> lists = new ArrayList<>();
                        lists.add(randomShoppingList);
                        DataStore.SHOPPING_LISTS_BY_TAG.put(randomShoppingList.tag, lists);
                    } else {
